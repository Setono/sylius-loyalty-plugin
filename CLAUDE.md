...
- **Repositories are Sylius resource-bundle repositories, never plain Doctrine/ORM services.** The impl extends `Sylius\Bundle\ResourceBundle\Doctrine\ORM\EntityRepository`, the interface extends `Sylius\Component\Resource\Repository\RepositoryInterface<Model>`, and the repository is wired by registering its resource in `Configuration` (`addResource(..., $repositoryClass, $interface)`) — not hand-registered in `services.xml`. Custom query methods use `$this->createQueryBuilder(...)`. Even the STI-root transaction repository is registered this way (the discriminator-map listener skips the abstract root). Alias the interface to the `setono_sylius_loyalty.repository.<name>` service.
...
- **Form testing (required for every form type)** - Follow Symfony's form unit-testing guidelines: https://symfony.com/doc/6.4/form/unit_testing.html
  - Extend `Symfony\Component\Form\Test\TypeTestCase` for form type tests
  - Use `$this->factory->create()` to build the form, then `$form->submit([...])` and assert `$form->isSynchronized()` plus the mapped model data
  - Test submission, validation, and data transformation, including empty and edge-case input
