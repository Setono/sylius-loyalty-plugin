...
The shop must be seeded. From `tests/Application` (dev env):

```bash
APP_ENV=dev bin/console doctrine:schema:create
APP_ENV=dev bin/console sylius:fixtures:load -n            # channels, customers, products
APP_ENV=dev bin/console sylius:fixtures:load setono_loyalty -n   # loyalty balances (see config/packages/setono_sylius_loyalty_fixtures.yaml)
